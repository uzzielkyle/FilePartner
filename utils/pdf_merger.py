...
class PdfMerger:
    ...
    def get_pdfs(root_dir: list) -> list:
        ...
        for row_num, folder in enumerate(root_dir):
            ...
            str_path = os.path.join(folder, '*.pdf')
            # also check if the glob is a pdf file, it may sometimes be a folder that ends with .pdf
            pdfs = [path.replace(
                "\\", "/") for path in glob.glob(str_path, recursive=True) if os.path.isfile(path)]
    ...
    def flatten_array(self, arr: list) -> list:
        """
        ...
        for element in arr:
            if isinstance(element, list):
                flattened_list.extend(self.flatten_array(element))
            else:
    ...
    def merge_files(self, main_path: str, root_dir: list) -> None:
        ...
        for row_num, folder in enumerate(root_dir):
            ...
            # .replace("\\","/") for 'slash' consistency... https://stackoverflow.com/a/18776536, tnx
            files = files + pdfs
            number_of_pages = self.get_num_pages(pdfs)

        files = self.flatten_array(files)
        total_pages = self.get_num_pages(files)
        # by default, save the output to the first folder
        output_path = os.path.join(root_dir[0], 'output.pdf')
        self.merge_pdf(files, output_path)
    ...
    def deep_merging(self, main_path: str, dirs: list) -> tuple[list]:
        outputs = []

        for folder in dirs:
            # Use os.path.join for paths
            str_path = os.path.join(folder, '*.pdf')
            pdfs = [path for path in glob.glob(str_path, recursive=True) if os.path.isfile(
                path) and not os.path.basename(path).startswith('output')]

            number_of_pages = self.merge_pdf(
                pdfs, os.path.join(folder, 'output.pdf'))

            if number_of_pages == 0:
                continue

            folder_path = f"📁 {os.path.normpath(folder)}"
            link = os.path.join(folder, 'output.pdf')

            outputs.append((link, folder_path, number_of_pages))
