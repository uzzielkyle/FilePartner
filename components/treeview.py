...
class TreeView(CTkFrame):
    ...
    def __init__(self, *args, 
        ...
        self.style.configure("mystyle.Treeview", font=('Arial', 11), rowheight=30)
        
        self.tree = ttk.Treeview(self, style='mystyle.Treeview', selectmode='browse')
        self.tree['columns'] = ('Link', 'Folder', 'Pages')
                
        self.tree.column('#0', width=0, stretch=False)
        self.tree.column('Link', width=0, stretch=False)
        self.tree.column('Folder', anchor='w', minwidth=int(self.WIDTH*1.65), stretch=True)
        self.tree.column('Pages', anchor='center', minwidth=int(self.WIDTH*.5), stretch=False)
    ...
    def on_double_click(self, event):
        try:
            ...
            subprocess.run([self.FILEBROWSER_PATH, '/select,', os.path.normpath(path)])

        except:
            self.blank_searchbar_message = CTkMessagebox(fg_color='whitesmoke', bg_color='whitesmoke', title='Link Error' ,message='Cannot locate file.', icon='warning', justify='center',font=('Arial', 11), option_focus='option_1', sound=True)
